...
f4 = coinbene_filename = 'coinbene-trades-{}.json'.format(date1)
...
ex_list = ['poloniex', 'binance', 'coinbase', 'coinbene']
sum_of_size = [
    sum(poloniex_df['size']),
    sum(binance_df['size']),
    sum(coinbase_df['size']),
    sum(coinbene_df['size'])
]
monthly_total_visit = [25*10**5, 205*10**5, 227*10**5, 27*10**4]
...
# plot_trade_size(poloniex_df, 2, 100)
# plot_trade_size(binance_df, 2, 100)
# plot_trade_size(coinbase_df, 2, 100)
plot_trade_size(coinbene_df, 2, 100)
