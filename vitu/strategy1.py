...
# 配置单/多账户初始持仓信息
ai.create_account(name='account_name_1', exchange='binance', 
                  account_type='digital.spot',
                  position_base=[{'asset': 'BTC', 'qty': 10}, \
                                 {'asset': 'USDT', 'qty': 200000}])

ai.create_account(name='account_name_2', exchange='binance',
                  account_type='digital.spot', 
                  position_base=[{'asset': 'ETH', 'qty': 200}])
...
def handle_data(context):
    ...
    close_price_1 = context.history(
        'BTC/USDT.binance', 'close', bars=context.MA_length, rtype='ndarray')
    # 计算MA值
    ...
    # 如果跌破10日均线卖出0.2BTC
        ...
        log.info("价格跌破10日均线, 买入BTC, 价格：%d" % (current_price_1))

    # 获取binance交易所的ETH/USDT历史数据
    close_price_2 = context.history(
        'ETH/USDT.binance', 'close', bars=context.MA_length, rtype='ndarray')
    # 计算MA值
    ...
    if (current_price_2 < MA10_2):
        context.account_2.buy("BTC/USDT.binance", current_price_2, 1)
        log.info("价格突破10日均线, 买入ETH, 价格：%d" % (current_price_2))
    # 如果跌破10日均线卖出1ETH
    elif (current_price_2 > MA10_2):
        context.account_2.sell("BTC/USDT.binance", current_price_2, 1)
